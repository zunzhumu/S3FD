    ...
    if (num_valid_gt > 0) {
      ...
      for(int k = 0; k < num_valid_gt; ++k){
      	...
      	for(int i = 0; i < num_anchors; ++i){
            if(max_matches[i].second == k)
               std::cout << "the best anchor:" << i << std::endl;
        }
      }
      ...
      // to compute the anchor mathed
      /*int total = 0;
      std::cout << "total gt number:" << num_valid_gt << std::endl;
      std::cout << "total anchors number:" << num_anchors << std::endl;
      for(int k = 0; k < num_valid_gt; ++k){
      	int count = 0;
      	for(int i = 0; i < num_anchors; ++i){
            if(max_matches[i].second == k)
        ...
        total += count;
        std::cout << "gt mathed anchor number:" << count << std::endl;
      }
      std::cout << "averge gt mathed anchor number:" << (float)total/num_valid_gt << std::endl;
