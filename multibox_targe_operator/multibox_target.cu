   ...
   /*if (threadIdx.x == 0) {
        for(int i = 0; i < num_labels;++i){
          for(int j = 0; j < num_anchors; j++){
                if(best_matches[j] == i) printf("the best anchor:%d\n", j);
             }  
        } 
     }*/
}
...
__global__ void FindGoodMatches(DType *best_matches, DType *anchor_flags,
  ...
  anchor_flags += nbatch * num_anchors;
  const int num_threads = kMaxThreadsPerBlock;
  __shared__ int indices[kMaxThreadsPerBlock];
  __shared__ int count_value[kMaxThreadsPerBlock];
  int ave_N = 0;
  for (int i = threadIdx.x; i < num_anchors; i += num_threads) {
    if (anchor_flags[i] < 0) {
      int idx = -1;
  ...
  //compute the anchor mathed
   __syncthreads();
    ...
    for(int i = threadIdx.x; i < num_labels; i += num_threads){
      ...
      // if(count > 0){
      //   printf("stage one: each gt matched anchor number:%d\n", count);
      // }
   }
   indices[threadIdx.x] = index;
   count_value[threadIdx.x] = count;
   __syncthreads();
   if(threadIdx.x == 0){
   num_gt_valid = 0;
   int total = 0;
   for(int i = 0; i < num_threads; ++i){
      if(num_gt_valid < indices[i]) num_gt_valid = indices[i];
      total += count_value[i];
   }
   ave_N = (float)total/num_gt_valid;
   //printf("the ground truth number:%d\n", num_gt_valid);
   //printf("the averge anchors matched number:%d\n", ave_N);
  }
   __syncthreads();
 
 // deal with tiny and outer face
 if(ave_N > 1 && threadIdx.x == 0){
    //printf("deal with tiny and outer face\n");
    float tiny_overlap = 0.1f;
    for(int i = 0; i < num_gt_valid; ++i){
         float max_value[kMaxThreadsPerBlock] = {0};
         int max_indice[kMaxThreadsPerBlock] = {0};
         int num = 0;
         for(int j = 0; j < num_anchors; ++j){
         DType temp = overlaps[j * num_labels + i];
         if (temp > tiny_overlap) {
          if(num < ave_N){
             max_value[num] = temp;
             max_indice[num] = j;
             num++;
             ...
             for(int k = 0; k < ave_N; ++k){
                 if(min_value > max_value[k]){
                     min_value = max_value[k];
                     min_index = k;
                  }
              }
              if(temp > min_value){
              max_value[min_index] = temp;//update the top ave_N 
              max_indice[min_index] = j;
              }
          }
        }//end if
      }//end j
       for(int k = 0; k < ave_N; ++k){
          if(anchor_flags[max_indice[k]] < 0 && max_indice[k] >= 0){
              best_matches[max_indice[k]] = i;
              anchor_flags[max_indice[k]] = 0.9f;
           }      
       }
    }//end i
  }//end if 
  //compute the anchor mathed again
   __syncthreads();
  /*  for(int i = threadIdx.x; i < num_labels; i += num_threads){
         int count = 0;
         for(int j = 0; j < num_anchors; ++j){
       ...
       if(count > 0){
         printf("stage two: each gt matched anchor number:%d\n", count);
       }
   } */ 
}
...
inline void MultiBoxTargetForward(const Tensor<gpu, 2, DType> &loc_target,
  ...
  ///int *max_indices_y;
  //float *max_values;
 // cudaMalloc(&max_indices_x, sizeof(int)*num_batches*num_anchors);
 // cudaMalloc(&max_indices_y, sizeof(int)*num_batches*num_anchors);
 // cudaMalloc(&max_values, sizeof(float)*num_batches*num_anchors);
  ...
  MULTIBOX_TARGET_CUDA_CHECK(cudaPeekAtLastError());
  //Free spaces
 // cudaFree(max_indices_x);
 // cudaFree(max_indices_y);
 // cudaFree(max_values);
