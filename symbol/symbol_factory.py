...
def get_config(network, data_shape, **kwargs):
    ...
    if network == 'vgg16_reduced':
        if data_shape >= 448:
            from_layers = ['relu3_3', 'relu4_3', 'relu5_3', '', '', '']
            num_filters = [256, 512, 512, 512, 256, 256]
            strides = [-1, -1, -1, 2, 2, 2]
            pads = [-1, -1, -1, 1, 1, 1]
            sizes = [[16.0 / data_shape], [32.0 / data_shape], [64.0 / data_shape], [128.0 / data_shape],
                     [256.0 / data_shape], [512.0 / data_shape]]
            ratios = [[1], [1], [1], [1], [1], [1]]
        ...
        num_layers = 50
        image_shape = '3,608,608'  # resnet require it as shape check
        network = 'resnet'
        from_layers = ['_plus2', '_plus6', '_plus12', '_plus15', '', '']
        num_filters = [-1, -1, -1, -1, 256, 128]
        strides = [-1, -1, -1, -1, 2, 2]
        pads = [-1, -1, -1, -1, 1, 1]
        sizes = [[16.0/data_shape], [32.0/data_shape], [64.0/data_shape], [128.0/data_shape], [256.0/data_shape], [512.0/data_shape]]
        ratios = [[1], [1], [1], [1], [1], [1]]
        normalizations = -1
        steps = []
        return locals()
