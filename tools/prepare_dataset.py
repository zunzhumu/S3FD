...
def load_widerface(image_set, devkit_path, shuffle=False):
    """
    ...
    imdbs = []
    for s in image_set:
        imdbs.append(WiderFace(s, devkit_path, shuffle, is_train=True))
    if len(imdbs) > 1:
...
def parse_args():
    ...
    parser.add_argument('--root', dest='root_path', help='dataset root path',
                        default=os.path.join(data_path),
                        type=str)
...
if __name__ == '__main__':
    ...
    if args.dataset == 'pascal':
        ...
        db = load_widerface(args.set, args.root_path, args.shuffle)
        print("saving list to disk...")
        db.save_imglist(args.target, root=args.root_path)
    else:
